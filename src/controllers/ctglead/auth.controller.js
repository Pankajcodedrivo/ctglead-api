...
// CTG Lead Login
const leadLogin = catchAsync(async (req, res, next) => {
    const user = await service.loginUser(req.body.email, req.body.password);
    if (user.role !== 'agency') {
        throw new ApiError('The user must be an agency owner to log in to this portal', 403);
    }
...
// Refresh Token
const refreshTokenslead = catchAsync(async (req, res, next) => {
    let data;
    try {
